...
void Adafruit_INA219::setCalibration_10V_50A() {
	//VBUS_MAX = 10 V
	//RSHUNT=0.01
	...
	//2). MaxExpected_I = 51.1984375 A (max expected current)


 	// 3. Calculate possible range of LSBs (Min = 15-bit, Max = 12-bit)
  	// MinimumLSB = MaxExpected_I/32767
  	// MinimumLSB = 0.0015259254738 
  	// MaximumLSB = MaxExpected_I/4096
 	// MaximumLSB = 0.01220703125


  	// 4. Choose an LSB between the min and max values
  	//    (Preferrably a roundish number close to MinLSB)
  	// CurrentLSB = 0.0015625
	...
	// 7. Compute the maximum current and shunt voltage values before overflow
  	// Max_Current = Current_LSB * 32767
  	// Max_Current =  51.1984375
	
	// 8. Compute the Maximum Power
  	// MaximumPower = Max_Current_Before_Overflow * VBUS_MAX
  	// MaximumPower = 51.1984375  * 10V
  	// MaximumPower = 511.984375 W


	// Set multipliers to convert raw current/power values
  	ina219_currentDivider_mA = 640;    
  	ina219_powerMultiplier_mW = 31.25; 
...
void Adafruit_INA219::setCalibration_10V_6A() {
  // Constants
  float shuntResistor = 0.01; // Ohms
  float maxExpectedCurrent = 6.0; // Amps

  // Choose a Current LSB (between min and max range)
  float currentLSB = 0.0002; // 0.2 mA per bit

  // Calculate calibration register
  uint16_t calibrationValue = (uint16_t)(0.04096 / (currentLSB * shuntResistor));

  // Set calibration
  ina219_calValue = calibrationValue;
  ina219_currentDivider_mA = (int)(1.0 / currentLSB); // = 5000
  ina219_powerMultiplier_mW = 20.0 / ina219_currentDivider_mA; // = 0.004
  ...
  // Set config register
  uint16_t config = INA219_CONFIG_BVOLTAGERANGE_16V |
                    INA219_CONFIG_GAIN_1_40MV | // Gain for up to ±40 mV
...
void Adafruit_INA219::setCalibration_10V_30A() {
  // Constants
  float shuntResistor = 0.01; // Ohms
  float maxExpectedCurrent = 30.0; // Amps

  // Choose a Current LSB (between min and max range)
  float currentLSB = 0.001; // 1 mA per bit

  // Calculate calibration register
  uint16_t calibrationValue = (uint16_t)(0.04096 / (currentLSB * shuntResistor)); // = 4096

  // Set calibration
  ina219_calValue = calibrationValue;
  ina219_currentDivider_mA = (int)(1.0 / currentLSB); // = 1000
  ina219_powerMultiplier_mW = 20.0 / ina219_currentDivider_mA; // = 0.02
  ...
  // Set config register
  uint16_t config = INA219_CONFIG_BVOLTAGERANGE_16V |
                    INA219_CONFIG_GAIN_8_320MV | // Gain for up to ±320 mV
                    INA219_CONFIG_BADCRES_12BIT |
                    INA219_CONFIG_SADCRES_12BIT_1S_532US |
                    INA219_CONFIG_MODE_SANDBVOLT_CONTINUOUS;
